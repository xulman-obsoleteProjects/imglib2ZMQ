...
public class ImgStreamer<T extends NativeType<T>> {
	...
	private ImgPlus<T> imgP;

	private byte[] headerBytes;
	private ProgressCallback log;

	public ImgStreamer(final ImgPlus<T> imgP, final ProgressCallback log) {
		this.imgP = imgP;
		ByteArrayOutputStream bos = createHeaders(imgP);
		this.headerBytes = bos.toByteArray();
		this.log = log;
	}

	public long getLength() {
		return headerBytes.length + computeLenghtOfImg();
	}
	...
	public void writeTo(OutputStream os) throws IOException {
		...
		try {
			...
			// and try to send everything (first the human readable payload, then raw
			// voxel data)
			Img<T> img = getUnderlyingImg(imgP);
			if (img.size() == 0) throw new RuntimeException(
				"Refusing to send an empty image...");
			...
			if (img instanceof ArrayImg) {
				...
				packAndSendArrayImg((ArrayImg<T, ? extends ArrayDataAccess<?>>) img,
					oos);
			}
				...
				// NB: The number of planes is deterministically given by the image
				// size/dimensions.
				...
				if (log != null) log.info("sending the image...");
				// packAndSendPlusData(imgP, socket);
				packAndSendPlanarImg((PlanarImg<T, ? extends ArrayDataAccess<?>>) img,
					oos);
			}
				...
				"Cannot determine the type of image, cannot send it.");
	...
	private int computeLenghtOfImg() {
		// TODO Auto-generated method stub
		return 0;
	}

	private ByteArrayOutputStream createHeaders(final ImgPlus<T> imgP) {
	...
	public static <T extends NativeType<T>> void packAndSend(
		final ImgPlus<T> imgP, final ObjectOutputStream os,
		final ProgressCallback log) throws IOException
	{
		Class<?> voxelClass = imgP.firstElement().getClass();
		if (!SUPPORTED_VOXEL_CLASSES.contains(voxelClass))
			throw new IllegalArgumentException("Unsupported voxel type, sorry.");
	...
	public static ImgPlus<?> receiveAndUnpack(final ObjectInputStream is,
		final ProgressCallback log) throws IOException, ClassNotFoundException
	{
		...
		if (!headerST.nextToken().startsWith("v1")) throw new RuntimeException(
			"Unknown protocol, expecting protocol v1.");
		...
		Img<? extends NativeType<?>> img = createImg(dims, backendStr,
			createVoxelType(typeStr));

		if (img == null) throw new RuntimeException(
			"Unsupported image backend type, sorry.");

		// the core Img is prepared, lets extend it with metadata and fill with
		// voxel values afterwards
		...
		if (img.size() == 0) throw new RuntimeException(
			"Refusing to receive an empty image...");
		...
		else if (backendStr.startsWith("PlanarImg")) {
			if (typeStr.contains("Byte")) receiveAndUnpackPlanarByteImg(
				(PlanarImg) img, is);
			else if (typeStr.contains("Short")) receiveAndUnpackPlanarShortImg(
				(PlanarImg) img, is);
			else if (typeStr.contains("Float")) receiveAndUnpackPlanarFloatImg(
				(PlanarImg) img, is);
			else if (typeStr.contains("Double")) receiveAndUnpackPlanarDoubleImg(
				(PlanarImg) img, is);
			// else: should not happen, see createVoxelType() above
		}
		else if (backendStr.startsWith("CellImg")) {
			throw new RuntimeException("Cannot receive CellImg images yet.");
		}
		else throw new RuntimeException("Unsupported image backend type, sorry.");
	...
	protected static <T extends NativeType<T>> void packAndSendArrayImg(
		final ArrayImg<T, ? extends ArrayDataAccess<?>> img,
		final ObjectOutputStream os) throws IOException
	{
		os.writeObject(img.update(null).getCurrentStorageArray());
	}

	protected static <T extends NativeType<T>> void receiveAndUnpackArrayImg(
		final ArrayImg<T, ? extends ArrayDataAccess<?>> img,
		final ObjectInputStream is) throws IOException, ClassNotFoundException
	{
		img.update(is.readObject());
	}

	protected static void packAndSendPlanarImg(
	...
	protected static void receiveAndUnpackPlanarByteImg(
		final PlanarImg<? extends NativeType<?>, ByteArray> img,
		final ObjectInputStream is) throws IOException, ClassNotFoundException
	{
		for (int slice = 0; slice < img.numSlices(); ++slice)
	...
	protected static void receiveAndUnpackPlanarShortImg(
		final PlanarImg<? extends NativeType<?>, ShortArray> img,
		final ObjectInputStream is) throws IOException, ClassNotFoundException
	{
		for (int slice = 0; slice < img.numSlices(); ++slice)
	...
	protected static void receiveAndUnpackPlanarFloatImg(
		final PlanarImg<? extends NativeType<?>, FloatArray> img,
		final ObjectInputStream is) throws IOException, ClassNotFoundException
	{
		for (int slice = 0; slice < img.numSlices(); ++slice)
	...
	protected static void receiveAndUnpackPlanarDoubleImg(
		final PlanarImg<? extends NativeType<?>, DoubleArray> img,
		final ObjectInputStream is) throws IOException, ClassNotFoundException
	{
		for (int slice = 0; slice < img.numSlices(); ++slice)
	...
	private static <Q> Img<Q> getUnderlyingImg(final Img<Q> img) {
		if (img instanceof Dataset) return (Img<Q>) getUnderlyingImg(((Dataset) img)
			.getImgPlus());
		else if (img instanceof WrappedImg) return getUnderlyingImg(
			((WrappedImg<Q>) img).getImg());
		else return img;
	}

	@SuppressWarnings("rawtypes") // use raw type because of insufficient support
																// of reflexive types in java
	private static NativeType createVoxelType(String typeStr) {
		for (Class<? extends NativeType> aClass : SUPPORTED_VOXEL_CLASSES)
			if (typeStr.startsWith(aClass.getSimpleName())) try {
	...
	private static <T extends NativeType<T>> Img<T> createImg(int[] dims,
		String backendStr, T type)
	{
		if (backendStr.startsWith("ArrayImg")) return new ArrayImgFactory<T>()
			.create(dims, type);
		if (backendStr.startsWith("PlanarImg")) return new PlanarImgFactory<T>()
			.create(dims, type);
		if (backendStr.startsWith("CellImg")) return new CellImgFactory<T>().create(
			dims, type);
